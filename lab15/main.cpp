...
//задание 6 10 вариант: Занести в новый файл слова, начинающиеся с согласной буквы.
...
bool isRussianOrEnglishLetter(wchar_t c) {
    return (c >= L'А' && c <= L'я') || c == L'Ё' || c == L'ё' || (c >= L'A' && c <= L'Z') || (c >= L'a' && c <= L'z');
}

bool isConsonant(wchar_t c) {
    // Перечисляем и маленькие, и большие гласные
    wstring vowels = L"аеёиоуыэюяАЕЁИОУЫЭЮЯaeiouyAEIOUY";
    return isRussianOrEnglishLetter(c) && vowels.find(c) == wstring::npos;
}

void processMyVariant(const string& fileName) {
    ifstream inFile(fileName);
    ofstream outFile("ConsonantWords.txt");
    ...
    if (!outFile.is_open()) {
        cout << "Ошибка: не удалось создать файл ConsonantWords.txt" << endl;
        return;
    }
    ...
    while (inFile >> word) {
        wstring wideWord = utf8ToWide(word);
        wstring cleanWord = L"";

        // Убираем из слова все знаки препинания и оставляем только буквы.
        ...
        // Если первая буква согласная, записываем слово в новый файл.
        if (!cleanWord.empty() && isConsonant(cleanWord[0])) {
            outFile << wideToUtf8(cleanWord) << endl;
            found = true;
        }
    }
    ...
    if (found) {
        cout << "Слова, начинающиеся с согласной буквы, записаны в файл ConsonantWords.txt" << endl;
    } else {
        cout << "Слова, начинающиеся с согласной буквы, не найдены." << endl;
    }
}
